...
bool getYesOrNo(string prompt, bool defaultValue)
{
    string response;
    std::cout << prompt + ": ";
	...
	*/
    ...
    while (!finished)
    {
        if (glfwGetTime() > 1.0f / render_fps && !errors_found)
        {
            ...
            std::string camPosStr = "camera_position";
            context->setUniform3fv(camPosStr.c_str(), 1, camera_pos);
            ...
            if (keys->checkPress(GLFW_KEY_HOME, true) && !keys->checkShiftHold())
            {
                ...
                std::cout << "focal length: " << glm::length(camera->getFocus() - camera->getPosition()) << std::endl;
            }
            ...
            if (keys->checkPress(GLFW_KEY_0, false))
            {
                ...
                settings.auto_tracking ? std::cout << "auto tracking enabled" << std::endl : std::cout << "auto tracking disabled" << std::endl;
            }
            ...
            if (keys->checkPress(GLFW_KEY_9, false))
            {
                ...
                camera->setTiltAngle(0.0f);
                camera->setRotateAngle(0.0f);
            }
